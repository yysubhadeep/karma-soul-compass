...
import { getDetailedQuestions } from '@/data/detailedQuestionsData';
...
const SelfFutureContent = ({ archetype }: SelfFutureContentProps) => {
  const detailedQuestions = getDetailedQuestions(archetype);

  if (!detailedQuestions) {
    return (
      <div className="text-center p-6">
        <p className="text-gray-600">Detailed questions for {archetype} are coming soon...</p>
      </div>
    );
  }
        ...
        <TabsContent value="soul-mirror" className="space-y-4 mt-4">
          <Card className="border-purple-200 bg-gradient-to-r from-purple-50 to-indigo-50">
            ...
            <CardContent className="px-4 pb-4">
              ...
              <div className="space-y-4">
                {detailedQuestions.soulMirror.map((item, index) => (
                  <div key={index} className="bg-white/60 rounded-lg p-4 border border-purple-100">
                    <div className="flex items-start space-x-2 mb-3">
                      ...
                      <p className="text-gray-800 text-sm font-medium leading-relaxed">{item.question}</p>
                    </div>
                    <div className="ml-6 bg-purple-50 rounded-lg p-3 border-l-4 border-purple-300">
              ...
              <div className="mt-6 bg-gradient-to-r from-purple-100 to-indigo-100 rounded-lg p-4 border border-purple-200">
                ...
                <p className="text-xs text-purple-700">
                  Journal about 2-3 questions that resonate most deeply. Notice what emotions or memories arise as you reflect on both the questions and answers.
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <TabsContent value="dharma-compass" className="space-y-4 mt-4">
          <Card className="border-emerald-200 bg-gradient-to-r from-emerald-50 to-teal-50">
            ...
            <CardContent className="px-4 pb-4">
              ...
              <div className="space-y-4">
                {detailedQuestions.dharmaCompass.map((item, index) => (
                  <div key={index} className="bg-white/60 rounded-lg p-4 border border-emerald-100">
                    <div className="flex items-start space-x-2 mb-3">
                    ...
                    <div className="ml-6 bg-emerald-50 rounded-lg p-3 border-l-4 border-emerald-300">
                      <p className="text-gray-700 text-sm leading-relaxed">{item.answer}</p>
                    </div>
                  </div>
                ))}
              ...
              <div className="mt-6 bg-gradient-to-r from-emerald-100 to-teal-100 rounded-lg p-4 border border-emerald-200">
                <div className="flex items-center space-x-2 mb-2">
                  <Eye className="h-4 w-4 text-emerald-600" />
                  <h4 className="text-sm font-semibold text-emerald-800">Vision Practice</h4>
                </div>
                <p className="text-xs text-emerald-700">
                  Choose 3 questions that spark the strongest vision for your future. Visualize yourself living these answers and taking the suggested actions.
                </p>
              </div>
            </CardContent>
          </Card>
